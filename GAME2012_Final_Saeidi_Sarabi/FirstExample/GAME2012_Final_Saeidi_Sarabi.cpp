...
///////////////////////////////////////////////////////////////////////
//
// GAME2012 - Final Project
// Submission by Negin Saeidi (101261395) and Mehrara Sarabi (101247463)
//
///////////////////////////////////////////////////////////////////////
...
PointLight pLights[12] = {  { glm::vec3(5.0f, 13.1f, -2.5f), 15.0f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
						    { glm::vec3(47.5f, 13.1f, -2.5f), 15.5f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
							{ glm::vec3(47.5f, 12.1f, -21.5f), 15.0f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
							{ glm::vec3(47.0f, 12.1f, -28.5f), 15.0f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
							{ glm::vec3(5.0f, 13.1f, -45.0f), 15.0f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
							{ glm::vec3(47.5f, 13.1f, -45.0f), 15.0f, glm::vec3(1.0f, 0.8f, 0.6f), 5.0f },
							{ glm::vec3(49.5f, 0.0f, -21.5f), 2.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f },
							{ glm::vec3(49.5f, 0.0f, -28.5f), 2.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f },
							{ glm::vec3(49.8f, 0.0f, -2.0f), 2.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f },
							{ glm::vec3(49.8f, 0.0f, -45.5f), 2.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f },
							{ glm::vec3(55.5f, 3.1f, -21.5f), 5.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f },
							{ glm::vec3(55.5f, 3.1f, -28.5f), 5.0f, glm::vec3(1.0f, 0.8f, 0.6f), 30.0f } };

SpotLight sLight(glm::vec3(23.7001f, 65.2495f, - 25.1001f),	// Position.
...
void resetView()
{
	position = glm::vec3(107.0f, 10.0f, -24.0f);
...
void init(void)
{
	...
	// DOOR
	unsigned char* image10= stbi_load("door1.png", &width, &height, &bitDepth, 0);
	if (!image10) cout << "Unable to load file!" << endl;

	glGenTextures(1, &txt[11]);
	glBindTexture(GL_TEXTURE_2D, txt[11]);
	...
	// CYLINDER
	image = stbi_load("T_PlasterWall_D1.png", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;

	glGenTextures(1, &txt[12]);
	glBindTexture(GL_TEXTURE_2D, txt[12]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	image = stbi_load("id5_roof_short.png", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;

	glGenTextures(1, &txt[13]);
	glBindTexture(GL_TEXTURE_2D, txt[13]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	image = stbi_load("stone.png", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;

	glGenTextures(1, &txt[14]);
	glBindTexture(GL_TEXTURE_2D, txt[14]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	image = stbi_load("T_RoadTile_D.png", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;

	glGenTextures(1, &txt[15]);
	glBindTexture(GL_TEXTURE_2D, txt[15]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	glGenTextures(1, &txt[16]);
	glBindTexture(GL_TEXTURE_2D, txt[16]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
	...
	g_cone.SetMat(0.1, 16);
	g_prism1.SetMat(0.1, 16);
	g_cone1.SetMat(0.1, 16);
...
void DrawCastleWalls()
{
	...
	for (int i = 0; i < 8; i++) {
		...
		transformObject(glm::vec3(5.5f, 0.1f, 0.5f), Z_AXIS, -20.0f, glm::vec3(48.0f, 2.0f, -23.5f- 0.5 * i));
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(2.0f, 2.0f, 1.0f), Y_AXIS, 90.0f, glm::vec3(47.0f, 6.0f, -2.0f - 2 * i));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	for (int i = 0; i < 22; i++)
	{
...
void DrawMaze()
{
	...
	{	//2-1 wall
		...
		{
			g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
			transformObject(glm::vec3(1.38f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(9.5f + 1.38 * i, 0.0f, -7.5f));
			glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
		}
		for (int i = 0; i < 21; i++)
		{
			g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
			transformObject(glm::vec3(1.38f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(9.5f + 1.38 * i, 1.5f, -7.5f));
			glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
		}
		//2-2
		for (int i = 0; i < 19; i++)
...
void DrawRoom()
{
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(18.0f + 1.5 * i, 1.4f, -14.5f));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	for (int i = 3; i < 8; i++)
	{
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(18.0f + 1.5 * i, 2.9f, -14.5f));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	for (int i = 3; i < 8; i++)
	{
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(18.0f + 1.5 * i, 1.4f, -34.5f));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	for (int i = 6; i < 8; i++)
	{
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), X_AXIS, 0.0f, glm::vec3(18.0f + 1.5 * i, 2.9f, -34.5f));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	for (int i = 6; i < 8; i++)
	{
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), Y_AXIS, 90.0f, glm::vec3(18.0f, 1.4f, -14.5f - 1.5 * i));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	glBindTexture(GL_TEXTURE_2D, txt[8]);
	for (int i = 0; i < 13; i++)
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), Y_AXIS, 90.0f, glm::vec3(18.0f, 2.9f, -14.5f - 1.5 * i));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	//4
	glBindTexture(GL_TEXTURE_2D, txt[7]);
	...
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), Y_AXIS, 90.0f, glm::vec3(29.5f, 1.4f, -14.5f - 1.5 * i));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
	glBindTexture(GL_TEXTURE_2D, txt[8]);
	for (int i = 0; i < 13; i++)
	{
		g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
		transformObject(glm::vec3(1.5f, 1.5f, 0.5f), Y_AXIS, 90.0f, glm::vec3(29.5f, 2.9f, -14.5f - 1.5 * i));
		glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	}
...
void drawSteps()
{
	...
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(3.75f, 2.75f, 0.2f), Z_AXIS, 20.0f, glm::vec3(43.75f, 0.0f, -27.0f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(3.75f, 2.75f, 0.2f), Z_AXIS, 20.0f, glm::vec3(43.75f, 0.0f, -23.25f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(0.75f, 3.85f, 0.2f), Z_AXIS, 0.0f, glm::vec3(46.25f, 0.0f, -27.0f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(0.75f, 3.85f, 0.2f), Z_AXIS, 0.0f, glm::vec3(46.25f, 0.0f, -23.25f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(0.75f, 2.65f, 0.2f), Z_AXIS, 0.0f, glm::vec3(42.5f, 0.0f, -27.0f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(0.75f, 2.65f, 0.2f), Z_AXIS, 0.0f, glm::vec3(42.5f, 0.0f, -23.25f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	...
	//3
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(45.5f, 0.0f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(45.5f, 1.0f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(45.5f, 2.0f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	//2
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(44.0f, 0.0f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(44.0f, 1.0f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	//1
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(1.5f, 1.0f, 3.5f), X_AXIS, 0.0f, glm::vec3(42.5f, 0.2f, -26.75f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
...
void drawTowers()
{
	...
	g_prism1.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(3.0f, 9.0f, 3.0f), X_AXIS, 0.0f, glm::vec3(45.5f, 0.0f, -23.0f));
	glDrawElements(GL_TRIANGLES, g_prism1.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_prism1.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(3.0f, 9.0f, 3.0f), X_AXIS, 0.0f, glm::vec3(45.5f, 0.0f, -30.0f));
	glDrawElements(GL_TRIANGLES, g_prism1.NumIndices(), GL_UNSIGNED_SHORT, 0);

	glBindTexture(GL_TEXTURE_2D, txt[13]);

	g_cone1.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.0f, 3.0f, 4.0f), X_AXIS, 0.0f, glm::vec3(45.0f, 9.0f, -23.5f));
	glDrawElements(GL_TRIANGLES, g_cone1.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cone1.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.0f, 3.0f, 4.0f), X_AXIS, 0.0f, glm::vec3(45.0f, 9.0f, -30.5f));
	glDrawElements(GL_TRIANGLES, g_cone1.NumIndices(), GL_UNSIGNED_SHORT, 0);

	// Cones

	g_cone.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75f, 3.0f, 4.75f), X_AXIS, 0.0f, glm::vec3(2.5f, 10.0f, -4.55f));
	glDrawElements(GL_TRIANGLES, g_cone.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cone.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75, 3.0f, 4.75f), X_AXIS, 0.0f, glm::vec3(2.5f, 10.0f, -47.55f));
	glDrawElements(GL_TRIANGLES, g_cone.NumIndices(), GL_UNSIGNED_SHORT, 0);


	g_cone.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75f, 3.0f, 4.75f), X_AXIS, 0.0f, glm::vec3(44.5f, 10.0f, -4.55f));
	glDrawElements(GL_TRIANGLES, g_cone.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cone.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75f, 3.0f, 4.75f), X_AXIS, 0.0f, glm::vec3(44.5f, 10.0f, -47.55f));
	glDrawElements(GL_TRIANGLES, g_cone.NumIndices(), GL_UNSIGNED_SHORT, 0);
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, txt[5]);
	g_cube1.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(12.0f, 0.5f, 20.5f), X_AXIS, 0.0f, glm::vec3(18.0f, 0.9f, -34.5f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
	...
	glBindTexture(GL_TEXTURE_2D, txt[11]);
	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75f, 6.25f, 0.05f), Y_AXIS, 90.0f, glm::vec3(47.99f, 1.5f, -22.62f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);

	g_cube.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(4.75f, 6.25f, 0.05f), Y_AXIS, 90.0f, glm::vec3(46.75f, 1.5f, -22.62f));
	glDrawElements(GL_TRIANGLES, g_cube.NumIndices(), GL_UNSIGNED_SHORT, 0);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("GAME2012_Final_Saeidi_Sarabi");
